...
<section>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO users (user_id, name, email, phone_number, address) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("issss", $user_id, $name, $email, $phone_number, $address);
    ...
    <table border="5">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $user_id = $row['user_id']; // Fetch the user_id
                echo "<tr>
                    <td>" . $row['user_id'] . "</td>
                    <td>" . $row['name'] . "</td>
                    <td>" . $row['email'] . "</td>
                    ...
                    <td>" . $row['address'] . "</td>
                    <td><a style='padding:4px' href='delete users.php?user_id=$user_id'>Delete</a></td> 
                    <td><a style='padding:4px' href='update users.php?user_id=$user_id'>Update</a></td> 
                </tr>";
            }
        } else {
